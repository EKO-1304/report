...
<div class="main-container mx-auto" style="max-width:1200px;">
      ...
      <div class="page-header">
        <div class="row">
          <div class="col-md-12 col-sm-12">
            ...
            <form action="<?= route_to('manage-mobil-update') ?>" method="post"  enctype="multipart/form-data">
              <?= csrf_field() ?>
              <input type="hidden" name="random" value="<?= $dt["random"] ?>" >
              <div class="form-group  required mb-1">
                ...
                <select class="custom-select2 form-control <?php if(session('errors.sesi')){ echo "form-control-danger";}else{} ?>" name="sesi" style="width: 100%; height: 38px;">
                    ...
                    <option  value="<?= $se["id"] ?>" <?php if($dt["sesi"] == $se["id"]){ echo "selected";}else{} ?> ><?= $se["sesi"] ?></option>
                    <?php } ?>
                </select>
              ...
              <div class="form-group  required mb-1">
                ...
                <select id="resulttujuan" class="custom-select2 form-control <?php if(session('errors.tujuan')){ echo "form-control-danger";}else{} ?>" name="tujuan" style="width: 100%; height: 38px;">
                    ...
                    <option value="dalam" <?php if($dt["tujuan"] == "dalam"){ echo "selected";}else{} ?> >Dalam Kota</option>
                    <option value="luar" <?php if($dt["tujuan"] == "luar"){ echo "selected";}else{} ?> >Luar Kota</option>
                </select>
              ...
              <div class="form-group required mb-1">
                ...
                ?>
                <input class="form-control <?php if(session('errors.tanggal')){ echo "form-control-danger";}else{} ?>" type="datetime-local" value="<?= $tgl[0].'T'.$tgl[1] ?>" name="tanggal">
                <small class="form-control-feedback text-danger"><?= session('errors.tanggal') ?></small>
              </div>
              <div class="form-group required mb-1">
                <label class="starlabel">Nama Broker</label>
                <input class="form-control <?php if(session('errors.namabroker')){ echo "form-control-danger";}else{} ?>" type="text" value="<?= $dt["broker"] ?>" name="namabroker" placeholder="Nama Broker">
              ...
              <div class="form-group  mb-1">
                ...
                <input class="form-control <?php if(session('errors.namapendamping')){ echo "form-control-danger";}else{} ?>" type="text" value="<?= $dt["pendamping"] ?>" name="namapendamping" placeholder="Nama Pendamping">
              ...
              <div class="form-group required mb-1">
                ...
                <input class="form-control <?php if(session('errors.namanasabah')){ echo "form-control-danger";}else{} ?>" type="text" value="<?= $dt["calnas"] ?>" name="namanasabah" placeholder="Nama Calon Nasabah">
                <small class="form-control-feedback text-danger"><?= session('errors.namanasabah') ?></small>
              </div>
              <div class="form-group required mb-1">
                <label class="starlabel">Alamat</label>
                <input class="form-control <?php if(session('errors.alamat')){ echo "form-control-danger";}else{} ?>" type="text" value="<?= $dt["alamat"] ?>" name="alamat" placeholder="Alamat">
              ...
              <div class="form-group  required">
                ...
                <select class="custom-select2 form-control <?php if(session('errors.status')){ echo "form-control-danger";}else{} ?>" name="status" style="width: 100%; height: 38px;">
                    ...
                    <option value="0" <?php if($dt["status"] == 0){ echo "selected";}else{} ?> >Active</option>
                    <option value="3" <?php if($dt["status"] == 1){ echo "selected";}else{} ?> >Batalkan</option>
                </select>
                <small class="form-control-feedback text-danger"><?= session('errors.status') ?></small>
              </div>

              <button type="submit" class="btn btn-sm btn-success mt-3">Edit Pengajuan Mobil</button>
              <a href="<?= base_url("manage-mobil") ?>" class="btn btn-sm btn-dark text-light mt-3" >Kembali</a>
           </form>
 ...
 <script>
        ...
        function datadaerah(){ 
                ...
                var resulttujuan = "<?= $dt["tujuan"] ?>";
                var daerah = "<?= $dt["daerah"] ?>";
                var token = $('.txt_csrfname').val();   
                $.ajax({
                    url:"<?= base_url('manage-tujuan-mobil-json-2'); ?>",
                    type:"POST",
                    data:{<?= csrf_token() ?>:token,jenis:resulttujuan,daerah:daerah},
                    success:function(respond){
                        $("#resultdaerah").html(respond);
                    },
                    error:function(){
