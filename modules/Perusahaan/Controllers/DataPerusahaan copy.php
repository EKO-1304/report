...
class DataPerusahaan extends BaseController
{
    ...
    foreach ($list as $lists) {
        ...
        $row    = array();
        $row[]  = $no;
        $row[]  = '<a class="btn-link" href="javascript:void(0)" data-toggle="modal"  data-backdrop="static" data-target="#view'.$random.'">'.$lists->namanasabah.'</a>';
        $row[]  = $lists->fullname;
                    ...
                    <div class="modal fade" id="delete'.$random.'" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            ...
                            <div class="modal-footer mx-auto py-1">
                                    ...
                                    <a href="javascript:void(0)" id="'.$random.'" onclick="deletelaporan(this.id)" class="ml-3 btn btn-sm btn-danger">Delete</a>
                            </div>
                            </div>
                        </div>
                    </div>
    ...
    foreach ($list as $lists) {
        ...
        $action .= '
                    ...
                    // $row[]  = '<a class="btn-link" href="javascript:void(0)" data-toggle="modal"  data-backdrop="static" data-target="#view'.$random.'">'.$lists->namanasabah.'</a>'.$action;
	...
	{
       $dt = $this->db->table("x_laporan")->where("random",$random)->get()->getRowArray();
       $data = [
			'seg' => $this->request->uri->getSegments(),
			'pretitle' => 'Laporan',
			'title' => 'View Laporan Nasabah',
			'dt' => $dt,
		];
