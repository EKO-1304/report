        ...
        <div class="row">
          <div class="col-md-12 col-sm-12">
            ...
            <nav aria-label="breadcrumb" role="navigation">
              ...
              <a href="<?= base_url("manage-users") ?>" class="btn btn-dark btn-sm" style="width:100%" ><span class="icon-copy ti-back-left mr-2"></span>Kembali</a>
						</div>
					</div>
            ...
            <?= $this->include('Modules\Layout\Views\alert') ?>
              ...
              <form action="<?= route_to('manage-edit-users') ?>" method="post">
                ...
                <input type="hidden" name="random" value="<?= $users["random"] ?>" >
                  ...
                  <div class="row">
                      <div class="col-sm-6">
                        ...
                        <div class="form-group required">
                          ...
                          <input class="form-control <?php if(session('errors.fullname')){ echo "form-control-danger";}else{} ?>" type="text" value="<?= $users['fullname'] ?>" name="fullname" placeholder="Nama Lengkap">
                          <small class="form-control-feedback text-danger"><?= session('errors.fullname') ?></small>
                        </div>
                        <div class="form-group required">
                          <label class="starlabel">E-mail</label>
                          <input class="form-control  <?php if(session('errors.email')){ echo "form-control-danger";}else{} ?>" type="text" value="<?= $users['email'] ?>" name="email" placeholder="E-mail">
                          <small class="form-control-feedback text-danger"><?= session('errors.email') ?></small>
                        </div>
                        <div class="form-group required">
                          <label class="starlabel">Username</label>
                          <input class="form-control  <?php if(session('errors.username')){ echo "form-control-danger";}else{} ?>" type="text" value="<?= $users['username'] ?>" name="username" placeholder="Username">
                      ...
                      <div class="col-sm-6">
                        ...
                        <div class="form-group  required">
                          ...
                          <select class="custom-select2 form-control <?php if(session('errors.groups_access')){ echo "form-control-danger";}else{} ?>" name="groups_access" style="width: 100%; height: 38px;">
                            ...
                            <?php foreach($groups as $gr){ ?>
                              <option value="<?= $gr['id'] ?>" <?php if($users['group_id'] == $gr['id']){echo "selected";}else{} ?> ><?= $gr['description'] ?></option>
                            <?php } ?>
                          </select>
                        ...
                        <div class="form-group  required">
                          ...
                          <select class="custom-select2 form-control <?php if(session('errors.status_user')){ echo "form-control-danger";}else{} ?>" name="status_user" style="width: 100%; height: 38px;">
                            ...
                            <option value="1" <?php if($users['status'] == null){ echo "selected";}else{} ?> >Active</option>
                            <option value="0" <?php if($users['status'] == "banned"){ echo "selected";}else{} ?> >Non Active</option>
                          </select>
